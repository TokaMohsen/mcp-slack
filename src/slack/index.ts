...
const deleteMessageTool: Tool = {
  ...
  description: "Delete a specific message in Slack",
  inputSchema: {
    type: "object",
    properties: {
      channel_id: {
      ...
      try {
        ...
        switch (request.params.name) {
          ...
          case "slack_delete_message": {
            const args = request.params
              .arguments as unknown as { channel_id: string; timestamp: string };
            if (!args.channel_id || !args.timestamp) {
              throw new Error(
                "Missing required arguments: channel_id and timestamp",
              );
            }
